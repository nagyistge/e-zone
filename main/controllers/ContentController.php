...
class ContentController extends Common {
	...
	public function showAction() {
		...
		$prev_page = $this->db->where("catid = $catid AND id < $id AND status = 1")->order_by('id DESC')->get('content_'.SITE_ID)->row_array();	//上一篇文章
        $next_page = $this->db->where("catid = $catid AND id > $id AND status = 1")->order_by('id DESC')->get('content_'.SITE_ID)->row_array();	//下一篇文章
	    $this->view->assign(array(
	        'cat' => $cat,
	        'page'  => $page,
